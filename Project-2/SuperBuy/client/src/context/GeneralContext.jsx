...
const GeneralContextProvider = ({children}) => {
  ...
  const fetchCartCount = async () =>{
    ...
    if(userId){
      try {
        const response = await axios.get('http://localhost:8000/api/cart/fetch-cart');
...
const login = async () => {
  ...
  try {
    ...
    const res = await axios.post('http://localhost:8000/api/users/login', loginInputs);
  ...
  const register = async () =>{
    try {
      const res = await axios.post('http://localhost:8000/api/users/register', inputs);
