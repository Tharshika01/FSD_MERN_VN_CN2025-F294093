...
const AllOrders = () => {
  ...
  const fetchOrders = async () =>{
    await axios.get(`http://localhost:8000/api/orders/fetch-orders`).then(
      (response)=>{
...
const cancelOrder = async (orderId) => {
  try {
    const { data } = await axios.put('http://localhost:8000/api/orders/cancel-order', { orderId });
  ...
  const updateOrderStatus = async(id) =>{
    await axios.put('http://localhost:8000/api/orders/update-order-status', {id, updateStatus}).then(
      (response)=>{
